...
FUNC void ZS_Talk ()
{
	...
	if ( (!C_BodystateContains(self, BS_SIT)) || (!Npc_CanSeeNpc(self, hero)) )
	{
		...
		// ------ Spieler zu nah dran ------		warum gabs das nicht in G1 -Harri
		if (Npc_GetDistToNpc(other,self) < 80) 
		{
			AI_Dodge (self);
		};
...
FUNC INT ZS_Talk_Loop ()
{
    PrintDebugNpc(PD_ZS_LOOP,"ZS_Talk_Loop");
    		...
    		if Trainiert == 1 // Muß mit einem Hallo aus der ani gerissen werden -Harri
	{
	...
	// Notlösung um mit Inventabenutzung und oder entfernen aus einer "Dialogfalle" zu kommen -Harri
