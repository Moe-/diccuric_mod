...
{	
	...
	scemeName				=	"MAP";
	description			= "Umsonst";
	TEXT[0]				= "Das große Buch";
	////COUNT[0]			= ;
	TEXT[1]				= "Der Weg war umsonst";
	////COUNT[1]			= ;
	//TEXT[2]			= "";
	//COUNT[2]			= ;
	//TEXT[3] 			= "";
	...
	{   
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID, -1, "font_15_book.tga"	   			); 	// -1 -> all pages 
 					Doc_PrintLine	( nDocID,  0, ""					);										
					Doc_PrintLines	( nDocID,  0, "Der Weg war umsonst"	);
					Doc_PrintLine	( nDocID,  0, "--------------------"					);
						Doc_PrintLine	( nDocID,  0, ""					);			
					Doc_PrintLines	( nDocID,  0, "Das ist die Belohnung für alle die, die denken überall gibt es was zu holen. Nun freu dich auf den Rückweg.");
					...
					Doc_PrintLine	( nDocID,  1, "");
					//Absatz
					Doc_PrintLines	( nDocID,  1, " diese Seite fehlt ");
					Doc_Show		( nDocID );
	};

//***************************************

INSTANCE ItWr_Umsonst_02 (C_Item)
{	
	name 				=	"Zettel";
	...
	scemeName			=	"MAP";	
	on_state[0]			=	UseUmsonst_02;

	description			=	name;
	TEXT[1]				=	"Eine Seite aus dem Buch 'Der Weg war umsonst'";
	TEXT[2]				=	"";
	TEXT[3]				=	"";
	TEXT[4]				=	"";
};

FUNC VOID UseUmsonst_02()
{   
	var int nDocID;
	...
	nDocID = 	Doc_Create		()			  ;								 
				...
				Doc_SetFont 	(nDocID, -1, "font_10_book.tga");
				Doc_PrintLine	(nDocID,  0, "Seite 2");
				Doc_SetFont 	(nDocID, -1, "font_15_book.tga");
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLines	(nDocID,  0, "Oder war er doch nicht umsonst?");
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLines	(nDocID,  0, "");
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLines	(nDocID,  0, "HHuuuuwaaa.");
				Doc_Show		(nDocID);
};
//***************************************

INSTANCE ItWr_Pfad (C_Item)
{	
	name 				=	"Hinweis";
...
{   
	...
	nDocID = 	Doc_Create		()			  ;								 
				...
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLines	(nDocID,  0, "Folge den Pfad der Erleuchtung");
				Doc_PrintLine	(nDocID,  0, "");
				Doc_PrintLines	(nDocID,  0, "Oder, nur (runter)fliegen ist schöner");
				Doc_Show		(nDocID);
};

//---------------------------------------------------------------------

INSTANCE ItWr_Credits (C_ITEM)
{	
	name 					=	"Credits";
	...
	//TEXT[0]				= "";
	////COUNT[0]			= ;
	TEXT[1]				= "Wer hat an diesem Mod mitgewirkt?";
	////COUNT[1]			= ;
	//TEXT[2]			= "";
	//COUNT[2]			= ;
	//TEXT[3] 			= "";
	...
	TEXT[5]				= NAME_Value;
	COUNT[5]			= value;
	on_state[0]				=	UseCredits;
};

	FUNC VOID UseCredits()
	{   
		var int nDocID;
		...
		nDocID = 	Doc_Create		()			  ;								// DocManager 
					...
					Doc_SetFont 	( nDocID, -1, "font_10_book.tga"	   	); 	// -1 -> all pages 
 					Doc_PrintLine	( nDocID,  0, "");					
					Doc_PrintLines	( nDocID,  0, "  Mitwirkende"	);
					Doc_PrintLine	( nDocID,  0, "----------------");
					Doc_PrintLine	( nDocID,  0, "");			
					Doc_PrintLines	( nDocID,  0, "Sprachaufnahmen:");
					Doc_PrintLines	( nDocID,  0, "Technik: Rachid Kaiser");
					Doc_PrintLines	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Sprecher:");
					Doc_PrintLines	( nDocID,  0, "Timoth: Roberto Montuori");
					Doc_PrintLines	( nDocID,  0, "Gosslin: David Havers");
					Doc_PrintLines	( nDocID,  0, "Carl: Andreas Rohr");
					Doc_PrintLines	( nDocID,  0, "");
					Doc_PrintLines	( nDocID,  0, "Handlung + Dialoge : Andreas Rohr");
					Doc_PrintLines	( nDocID,  0, "Schlüsselquest + DIaloge :Harri Taubert");
					Doc_PrintLines	( nDocID,  0, "Naja, und den Rest was für so ein Mod noch nötig ist, ist auch von mir. Wie z.B. Mesh, Scripting, Animationen (die Bäume) und was sonst noch alles neu ist.");
					...
					//2.Seite
					Doc_SetMargins	( nDocID, -1, 30, 20, 275, 20, 1   		);  //  0 -> margins are in pixels (Position des Textes von den Ränder des TGAs aus, links,oben,rechts,unten)
					Doc_PrintLines	( nDocID,  1, "Ich hoffe dieses kleine Mod hat euch spaß gemacht. Mehr gibt es wenn unser Haupt-mod 'Diccuric' fertig ist. Die world hab ich komplett mit neuen Bäumen ausgetauscht");
					Doc_PrintLines	( nDocID,  1, "Ach ja da kommen wir gleich zum eigentlichen Zweck dieses Mod's");
					Doc_PrintLines	( nDocID,  1, "Ihr solltet euch doch bitte mal mit den Player in die 'Mitte' Wald bzw. Gras stellen und die fps messen. oder einen subjektiven Eindruk der Spielbarkeit geben");
					Doc_PrintLines	( nDocID,  1, "Bei Tests in der Diccuric-World fallen die fps z.T. bis auf 9 runter. Nun ist jeder PC etwas anders - darum dieser Performance-Test-Mod");
//					Doc_PrintLines	( nDocID,  1, "Im Forum werd ich dann noch ein Thema öffnen. Da könnt ihr mir dann antworten");
					Doc_PrintLine	( nDocID,  1, "");
					Doc_PrintLine	( nDocID,  1, "Danke für euer Interesse.");
					Doc_PrintLines	( nDocID,  1, "- Harri");
					Doc_Show		( nDocID );
	};
