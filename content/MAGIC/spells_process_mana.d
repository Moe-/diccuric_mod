...
func INT Spell_ProcessMana(VAR INT manaInvested)
{
	...
	if (Npc_GetActiveSpell(self) == SPL_WALAHEILEN			)	{	return  Spell_Logic_HeileWala			(manaInvested); };	
	if (Npc_GetActiveSpell(self) == SPL_SUMMONRAZOR		)	{	return	Spell_Logic_SummonRazor		(manaInvested);	};
	if (Npc_GetActiveSpell(self) == SPL_SUMMONHARPIE	)	{	return	Spell_Logic_SummonHarpie		(manaInvested);	};
};